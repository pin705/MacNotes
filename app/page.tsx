...
const MacNotes = () => {
  ...
  const handleAddNote = async () => {
    try {
      const res = await fetch('/api/notes', {
        ...
        body: JSON.stringify({ title: 'Ghi chú mới', body: '' })
      });
  ...
  const handleUpdateNote = (field: keyof NoteData, value: string) => {
      ...
      try {
        ...
        if (Array.isArray(importedNotes)) {
          if (confirm('Hành động này sẽ thêm các ghi chú vào CSDL. Bạn có chắc chắn không?')) {
            // Import sequentially to avoid race conditions or overload
    ...
    <div className={`flex h-screen w-full bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans overflow-hidden transition-colors duration-200`}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1 scrollbar-hide">
            {isLoading ? (
                <div className="flex justify-center mt-10 text-zinc-400 text-xs">
                    Đang tải...
                </div>
            ...
            filteredNotes.map(note => (
              <div 
                ...
                `}
              >
                <h3 className={`font-bold text-sm mb-1 truncate ${activeNoteId === note._id ? 'text-white' : 'text-zinc-900 dark:text-zinc-100'}`}>
                  {note.title || 'Ghi chú chưa đặt tên'}
                </h3>
                <div className="flex items-baseline gap-2">
                  <span className={`text-xs whitespace-nowrap ${activeNoteId === note._id ? 'text-yellow-100' : 'text-zinc-400'}`}>
                    {formatDate(note.updatedAt)}
                  </span>
                  <p className={`text-xs truncate flex-1 ${activeNoteId === note._id ? 'text-yellow-50' : 'text-zinc-500'}`}>
                ...
                <button 
                  ...
                  className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity
                    ${activeNoteId === note._id ? 'text-white hover:bg-yellow-600' : 'text-red-500 hover:bg-zinc-300 dark:hover:bg-zinc-700'}
                  `}
                >
                  <Trash2 size={16} />
                </button>
